...
async def info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lines = await _build_user_info_lines(update, context, chat, db_user, user_id)
    await _send_user_info_response(message, user_id, lines)


async def _build_user_info_lines(update, context, chat, db_user, user_id):
    ...
    if update.effective_chat.type in ("group", "supergroup"):
        await _add_group_info(lines, update, context, user_id, db_user)
...
async def _add_group_info(lines, update, context, user_id, db_user):
    try:
